...
public class Strings {
    ...
    /**
     * https://raw.githubusercontent.com/kellegous/perfboard/master/src/elemental/base/StringUtil.java
     */
    ...
    public static CharSequence clipText(CharSequence text, int maxLength, String ender) {
        CharSequence clippedText;
        if (isNullOrEmpty(text))
            return "";
        if (maxLength >= text.length())
            clippedText = text;
        else if (text.length() <= ender.length())
            clippedText = text;
        else if (maxLength <= ender.length())
            clippedText = text.subSequence(0, maxLength);
        else
            clippedText = text.subSequence(0, maxLength - ender.length()) + ender;
    ...
    public static String addQuestionMark(String question) {
        String s = question.trim();
        if (!s.substring(s.length() - 1, s.length()).contains("?")) {
            return s + " ?";
        } else {
            return s;
        }
